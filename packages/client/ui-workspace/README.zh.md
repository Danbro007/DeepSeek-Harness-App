...
视图菜单中的**已归档会话**按 Host 顺序显示注册表归档快照。每一行都可通过 `ctx.workspaces.unarchiveSession` 幂等恢复，并回到其保留的 Workspace 或 Ungrouped 位置。
...
- **没有 Session 删除**：归档与恢复均可用，但删除 Workspace 注册记录仍不会删除 Session。
