...
- `ctx.workspaceRegistry.archiveSession(id)` / `unarchiveSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot, so unarchive restores its position. Both mutations are idempotent; archiving accepts any live or persisted session (accounted or Ungrouped) and rejects an unknown id. State written before the field existed parses with an empty set.
