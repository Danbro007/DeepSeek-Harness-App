...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - macOS Electron 发行包装层通过受监督的回环 Web profile 复用浏览器应用；未来的无端口 Electron 客户端经由 IPC fetch 载体复用同一套 web client 包。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器访问与受监督的桌面发行包装层；零 workspace 依赖（注册表经结构注入到达）；无端口 Electron 客户端不复用它 |
