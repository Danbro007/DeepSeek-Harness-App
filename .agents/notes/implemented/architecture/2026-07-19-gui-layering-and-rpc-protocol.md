...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - The macOS Electron distribution wrapper reuses the browser application over a supervised loopback Web profile; a future port-free Electron client reuses the same web client packages over an IPC fetch carrier.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access and the supervised desktop distribution wrapper; zero workspace dependencies (the registry arrives by structural injection); a port-free Electron client does not reuse it |
