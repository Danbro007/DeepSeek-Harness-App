...
| `@deepseek-ai/dsh-tool-usage` | `deepseek_usage` | `ctx.tools`, `ctx.usage` | `tool/call`, `tool/result` | - | deepseek_usage reads the current balance and account usage. Missing credentials degrade to notices in the result instead of removing the tool schema. |
...
Read the DeepSeek platform account usage: current balance (from the API key) and the current month token usage, cost, and request counts (from the platform session token, when configured). Call when the user asks about DeepSeek usage, balance, or spending.
...
Source: [`packages/usage/tool-usage/src/index.ts`](../packages/usage/tool-usage/src/index.ts)
