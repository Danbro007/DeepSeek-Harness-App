...
flowchart LR
  ...
  pkg_usage_deepseek["usage-deepseek"]
  svc_usage["ctx.usage<br/>DeepSeek account usage reader"]
  pkg_tool_usage["tool-usage"]
...
| `ctx.usage` | `core` | [`usage-deepseek`](../packages/usage/usage-deepseek) | - | [`tool-usage`](../packages/usage/tool-usage), [`api-remotes`](../packages/api/remotes) | - | 通过逐操作解析凭据读取余额和私有控制台用量；每个来源都单独降级为提示。 |
