...
| `@deepseek-ai/dsh-tool-usage` | `deepseek_usage` | `ctx.tools`、`ctx.usage` | `tool/call`、`tool/result` | - | deepseek_usage 读取当前余额和账户用量。凭据缺失会在结果中降级为提示，而不会移除工具 schema。 |
...
<a id="deepseek-aidsh-tool-usage"></a>

## `@deepseek-ai/dsh-tool-usage`

### `deepseek_usage`

读取 DeepSeek 平台账户用量：当前余额（来自 API Key），以及已配置平台会话 token 时的当月 token 用量、费用和请求数。用户询问 DeepSeek 用量、余额或支出时调用。
...
来源：[`packages/usage/tool-usage/src/index.ts`](../packages/usage/tool-usage/src/index.ts)

deepseek_usage 读取当前余额和账户用量。凭据缺失会在结果中降级为提示，而不会移除工具 schema。
